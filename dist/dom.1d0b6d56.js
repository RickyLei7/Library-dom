...
window.dom = {
  create: function create(string) {
    var container = document.createElement('template'); //加template是因为比如div里面不能直接加td，违反html语法
  ...
  style: function style(node, name, value) {
    if (arguments.length === 3) {
      ...
      if (typeof name === 'string') // 可能会是想获取 dom.style(div, 'color')
      ...
      for (var key in object) {
        ...
        // node.style.color = ....
        // node.style.color = ....
        node.style[key] = object[key]; // 如果是.style.key的话，key是一个字符串，[key]是一个字符串
      }
    }
  },
  class: {
  ...
  find: function find(selector, scope) {
    return (scope || document).querySelectorAll(selector); //如果有scope，那就是scope.quer...没有scope,那就是document.....
    //高手写法
  },
